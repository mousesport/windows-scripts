...
set basename4=parking
set baseParking=/S %serv%\%basename4%

set log=%SRVput%\backup.log
set adminuser=/N backup /P 123456
...
rem  *******************************************************************
rem  Делаю запись в журнал о начале резервного копирования Parking
rem  *******************************************************************
echo ============ add events in logfile - start %basename4% ============ 
echo ============ START %basename4% %date% %time:~0,2%:%time:~3,2% ================== >> %log%

rem  *******************************************************************
rem  Подготавливаем базу Parking к резервному копированию
rem  *******************************************************************
echo start base ZUPsuk
...
rem  *******************************************************************
echo created folder?
IF exist %SRVput%\%basename4% ( echo %SRVput%\%basename4% exists ) ELSE ( mkdir %SRVput%\%basename4% && echo %SRVput%\%basename4% created)

rem  *******************************************************************
rem  Завершаем работу пользователей и блокируем базу
rem  *******************************************************************
echo kicked users
start "" /wait "%v8exe%" ENTERPRISE %baseParking% %adminuser% /DisableStartupMessages /C"ЗавершитьРаботуПользователей" /OUT"%log%" -NoTruncate

rem  *******************************************************************
rem  Начало резервного копирования базы Parking
rem  *******************************************************************
echo start backup base
start "" /wait "%v8exe%" CONFIG %baseParking% /DisableStartupMessages /DumpIB "%SRVput%\%basename4%\1c82_%basename4%_%date%.dt" %adminuser% /UC"КодРазрешения" /OUT"%log%" -NoTruncate

rem  *******************************************************************
rem  Разрешаем работу пользователей и разблокируем базу
rem  *******************************************************************
echo Enabled login users
start "" /wait "%v8exe%" ENTERPRISE %baseParking% %adminuser% /C"РазрешитьРаботуПользователей" /UC"КодРазрешения" /DisableStartupMessages /OUT"%log%" -NoTruncate

rem  *******************************************************************
rem  Делаю запись в журнал об окончании резервного копирования базы Parking
rem  *******************************************************************
echo add events in logfile
echo. >> %SRVput%\%error%
echo \\\\\\\\\\\\\\\\ END backup ZUPsuk %date% %time:~0,2%:%time:~3,2% \\\\\\\\\\\\\\ >> %log%
...
rem  *******************************************************************
net use %putbackup% \\10.255.254.2\backup\1c\1c8.2 /user:1cbackup 123456
